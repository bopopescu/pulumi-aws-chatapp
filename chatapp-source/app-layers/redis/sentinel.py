...
class SentinelConnectionPool(ConnectionPool):
    ...
    def owns_connection(self, connection):
        check = not self.is_main or \
                (self.is_main and
                 self.main_address == (connection.host, connection.port))
    ...
    def get_main_address(self):
        main_address = self.sentinel_manager.discover_main(
            self.service_name)
        if self.is_main:
            if self.main_address != main_address:
                self.main_address = main_address
                # disconnect any idle connections so that they reconnect
                # to the new main the next time that they are used.
